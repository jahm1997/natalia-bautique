...
import { brandsApi, productsApi, categoriesApi } from '../../lib/api';
...
export default function AdminDashboard({ onNavigate }: Props) {
  ...
  const loadStats = async () => {
    try {
      const [allBrands, allProducts, cats] = await Promise.all([
        brandsApi.listAll(),
        productsApi.listAll(),
        categoriesApi.list(),
      ]);
      setStats({
        ...
        activeBrands: allBrands.filter(b => b.is_active).length,
