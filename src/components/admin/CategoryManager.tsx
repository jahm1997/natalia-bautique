...
export default function CategoryManager() {
  ...
  const handleSave = async () => {
    ...
    let err;
    if (editId) {
      ({ error: err } = await supabase.from('categories').update(payload).eq('id', editId));
    } else {
      ...
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-cream/30 text-sm mt-0.5">
            {categories.length} categorías · Las marcas se organizan por categoría
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        <div className="space-y-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-16 bg-[#0f0f0f] animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="space-y-2">
          {categories.map(cat => (
            <div
              key={cat.id}
              className="flex items-center gap-4 bg-[#0f0f0f] border border-white/5 hover:border-white/10 px-5 py-4 transition-colors"
            >
              ...
              <div className="flex items-center gap-1 text-xs text-cream/25 flex-shrink-0">
                Orden: {cat.sort_order}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={() => openEdit(cat)}
                  className="flex items-center gap-1 text-xs text-cream/30 hover:text-gold transition-colors px-2 py-1"
                >
                  <Edit2 size={12} />Editar
                </button>
                <button
                  onClick={() => setConfirmDelete(cat.id)}
                  className="text-cream/15 hover:text-red-400 transition-colors px-2 py-1"
                >
                  <Trash2 size={13} />
                </button>
              </div>
          ...
          {categories.length === 0 && (
            <div className="text-center py-16">
              <p className="text-cream/20 text-sm">No hay categorías. Crea la primera.</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0f0f0f] border border-white/10 w-full max-w-md">
            ...
            <div className="p-6 space-y-5">
              <div>
                <label className="block text-cream/50 text-xs tracking-widest uppercase mb-1.5">Nombre</label>
                <input
                  value={form.name}
              ...
              <div>
                <label className="block text-cream/50 text-xs tracking-widest uppercase mb-1.5">Slug (URL)</label>
                <input
                  value={form.slug}
              ...
              <div>
                <label className="block text-cream/50 text-xs tracking-widest uppercase mb-1.5">
                  Icono (emoji)
                </label>
                <input
                  value={form.icon}
                  ...
                  maxLength={4}
                />
                <p className="text-cream/20 text-xs mt-1">Pega un emoji o deja vacío</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0f0f0f] border border-white/10 w-full max-w-sm p-6">
            ...
            <p className="text-cream/40 text-sm mb-6">
              Las marcas de esta categoría quedarán sin categoría. No se puede deshacer.
            </p>
