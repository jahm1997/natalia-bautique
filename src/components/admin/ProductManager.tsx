...
export default function ProductManager({ selectedBrandId, selectedBrandName, onChangeBrand }: Props) {
  ...
  const applyBulkPrice = async () => {
    ...
    await supabase.from('products').update({ price: Number(bulkPrice) }).eq('brand_id', selectedBrandId);
    setBulkPrice('');
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <select
          ...
          onChange={e => {
            ...
            if (b) onChangeBrand(b.id, b.name);
          }}
      ...
      {selectedBrandId && products.length > 0 && (
        <div className="mb-6 bg-[#0f0f0f] border border-white/5 p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-cream text-sm font-bold">Cambiar precio a todos los productos</p>
          </div>
          <div className="flex gap-3 items-center flex-wrap">
            <div className="relative">
              ...
              <input
                ...
                placeholder="35000"
                className="admin-input pl-7 max-w-[160px]"
              />
            </div>
            <span className="text-cream/30 text-xs">COP</span>
            <button
              onClick={applyBulkPrice}
              disabled={applyingBulk || !bulkPrice}
              className="flex items-center gap-2 bg-gold text-black font-bold px-4 py-2 text-xs uppercase tracking-wider hover:bg-cream transition-colors disabled:opacity-40"
            >
              {applyingBulk
          ...
          <p className="text-cream/20 text-xs mt-2">
            Actualizará el precio de los {products.length} productos de esta marca
          </p>
        </div>
      )}
      ...
      {loading ? (
        ...
        <>
          {paged.length > 0 ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {paged.map(p => (
                  <div
                    key={p.id}
                    className="bg-[#0f0f0f] border border-white/5 hover:border-white/10 transition-colors overflow-hidden group"
                  >
                    <div className="aspect-square overflow-hidden relative">
                      <img
                        src={p.image_url || 'https://images.pexels.com/photos/1152077/pexels-photo-1152077.jpeg?auto=compress&cs=tinysrgb&w=300'}
                        alt={p.name}
                      ...
                      {!p.is_active && (
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className="bg-black/60 text-cream/50 text-[10px] px-2 py-0.5 uppercase tracking-widest">Oculto</span>
                        </div>
                      )}
                    </div>
                    <div className="p-3">
                      <p className={`text-xs font-semibold leading-tight line-clamp-1 ${p.is_active ? 'text-cream' : 'text-cream/30'}`}>
                        {p.name}
                      </p>
                      ...
                      <div className="flex items-center gap-1 mt-2">
                        <button
                          onClick={() => openEdit(p)}
                          className="flex items-center gap-1 text-[11px] text-cream/30 hover:text-gold transition-colors"
                        >
                        ...
                        <button
                          ...
                          className={`text-[11px] transition-colors ${p.is_active ? 'text-green-400/60 hover:text-red-400' : 'text-red-400/50 hover:text-green-400'}`}
                        >
                        ...
                        <button
                          ...
                          className="text-cream/15 hover:text-red-400 transition-colors"
                        >
                          <Trash2 size={11} />
                        </button>
                      </div>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-white/5">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="w-8 h-8 border border-white/10 flex items-center justify-center text-cream/40 hover:text-gold hover:border-gold/30 transition-colors disabled:opacity-20"
                    >
                      <ChevronLeft size={14} />
                    </button>
                    <button
            ...
            <div className="text-center py-20">
              ...
              <button
                onClick={openCreate}
                className="flex items-center gap-2 bg-gold text-black font-bold px-5 py-2.5 text-xs uppercase tracking-widest hover:bg-cream transition-colors mx-auto"
              >
        ...
        <div className="text-center py-24">
          <p className="text-cream/15 text-sm">Selecciona una marca para ver sus productos</p>
        </div>
      )}
          ...
          <div className="bg-[#0f0f0f] border border-white/10 w-full max-w-lg max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 sticky top-0 bg-[#0f0f0f] z-10">
              <div>
                ...
                <p className="text-cream/30 text-xs mt-0.5">{selectedBrandName}</p>
              </div>
              <button onClick={() => setShowForm(false)} className="text-cream/30 hover:text-cream transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <Field label="Nombre del producto">
                <input
                  ...
                  placeholder="Bolso LV Neverfull"
                  autoFocus
                />
              </Field>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-cream/25 text-xs">Los clientes pueden ver y pedir este producto</p>
                </div>
              </label>
