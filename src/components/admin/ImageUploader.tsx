      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-1">
          <button
            type="button"
            onClick={() => setMode('url')}
            className={`flex items-center gap-1 px-2 py-1 text-xs transition-colors ${mode === 'url' ? 'bg-gold/20 text-gold' : 'text-cream/30 hover:text-cream/60'}`}
          >
            <Link size={11} />URL
          </button>
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 text-xs transition-colors ${mode === 'upload' ? 'bg-gold/20 text-gold' : 'text-cream/30 hover:text-cream/60'}`}
          >
          ...
          >
            {uploading ? (
              ...
              <>
                ...
                <span className="text-xs text-cream/20">JPG, PNG, WEBP · Máx 5MB</span>
              </>
            )}
          </button>
      ...
      {value && (
        <div className="mt-3 relative inline-block group">
          ...
          <button
            type="button"
            onClick={() => onChange('')}
            className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
          >
